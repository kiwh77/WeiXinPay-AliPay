...
-(instancetype)initWithParams:(NSDictionary *)params
{
    ...
    if (self) {
        ...
        Ivar * iv = class_copyIvarList([self class], &outCount);
        for (int i=0; i<outCount; i++) {
            Ivar ivar = iv[i];
            //这里取出来的属性名前面有一个下划线"_"，所以把第一位截掉
            NSString * ivarName = [[NSString stringWithUTF8String:ivar_getName(ivar)] substringFromIndex:1];
            if ([params.allKeys containsObject:ivarName]) {
                object_setIvar(self, ivar, [params objectForKey:ivarName]);
            }
        }
    }
    return self;
